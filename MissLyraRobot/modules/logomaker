...
from MissLyraRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    xx = await event.reply("`Bana rahi hu ruko thoda me bhi insaan hu machine nahi😶...`")
    name = event.pattern_match.group(1)
    if not name:
        await xx.edit("`Kuch text dega tab na logo banaungi 😏!\nExample: /logo <Chup Chap apne name likha>!`")
        return
    bg_, font_ = "", ""
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("./MissLyraRobot/resources/fonts/*")
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [TheXCodeTeam](https://t.me/TheXCodeTeam)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./MissLyraRobot/resources/fonts"):
            os.remove(font_)
