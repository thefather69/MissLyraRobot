...
import html
import requests
import MissLyraRobot.modules.sql.chatbot_sql as sql
...
from MissLyraRobot.modules.helper_funcs.chat_status import user_admin, user_admin_no_reply
from MissLyraRobot import dispatcher, updater, SUPPORT_CHAT
from MissLyraRobot.modules.log_channel import gloggable
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "Lyra Robot Chatbot Disabled By {}.".format(mention_html(user.id, user.first_name)),
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "Lyra Robot Chatbot Enabled By {}.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML,
            )
...
__help__ = """
✗ `Chatbot utilizes the` *Lyra* `api which allows Lyra to talk and provide a more interactive group chat experience.`
